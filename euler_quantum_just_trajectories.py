...
def read_config(path="config.txt"):
	...
	return int(params["num_traj"]), int(params["desired_jumps"])
...
if __name__ == '__main__':
    ...
    for gamma in range(3, 10):
        ...
        qwalks_path = f"Qwalks_gamma={gamma}_ntraj={num_traj}.txt"

        start_time = time.time()

        print(f"\n--- gamma = {gamma:.1f}, writing jumps to {qwalks_path}")

        with open(qwalks_path, 'w') as outfile:
            ...
            for node in G.nodes():
                ...
                # Write the trajectory for the current 'node' and 'gamma' to the file
                for jumps in trajs:
                    line = f"{node} " + " ".join(map(str, jumps)) + "\n"
                    outfile.write(line)
